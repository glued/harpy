...
function openDefaultBrowser(port){
    ...
    setTimeout(()=> new Runner(command,[`http:\/\/localhost:${port}/`], getENV(), null), 2000)
}

function createServerNote(port, key, detached = false){
  let serverNote = atom.notifications.addInfo('Harp server running', {
    detail:`on port ${port} ${detached ? ' detached' : ''}`,
    dismissable: true,
    buttons:[
      {
        text: 'Stop Server',
        className: 'btn-one',
        onDidClick: () => {
          if(detached) exec(`kill ${key}`)
          else running.get(key).stop()
          serverNote.dismiss()
        }
      }
    ]
...
export default{
  ...
  listActiveServers:(show = true) => {
    let ports = new Set()

    for(let [key, value] of running.entries()){
      createServerNote(value.port, key)
      ports.add(String(value.port))
    }

    if(!isWindows())
      exec('ps -ef | grep "harp"', (err, stdout, stderr) => {
        if(err) return
        const nodeProcesses = stdout.split('\n')
        for(let nodeProcess of nodeProcesses)
          if(nodeProcess.includes('/usr/local/bin/harp')){
            const info = nodeProcess.split(' ').filter(str => str !== '')
            const port = info[info.length - 1]
            if(!ports.has(port))
              createServerNote(port, info[1], true)
          }
      })
  },
  ...
  startMultihost:() => {
    ...
    portfinder.basePort = 9000
    portfinder.getPort((err, port) => {
      running.set(cwd, new Runner('harp', ['multihost', '--port', port], getENV(), cwd, port))
      note.success(`Harp multihost started`, `running on port ${running.get(cwd).port}`)
      openDefaultBrowser(port)
    })
  },
  ...
  surge:() => {
    const cwd = getCWD()
    running.set(cwd, new Runner('surge', [atom.config.get('harpy.compileFolder')], getENV(true), cwd, 3))
  },
