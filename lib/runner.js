...
function openDefaultBrowser(port){
    setTimeout(()=> new Runner('open',[`http:\/\/localhost:${port}/`], getENV(), null), 1000)
}
...
export default{
  ...
  startServer:() => {
    ...
    portfinder.getPort((err, port) => {
      ...
      openDefaultBrowser(port)
      // new Runner('open',[`http:\/\/localhost:${port}/`], getENV(), cwd)
    })
  },

  surge:() => {
    const cwd = getCWD()
    running.set(cwd, new Runner('surge', ['--build', '-p', './'], getENV(), cwd, 2))
  },
